...
const std::vector<double>& Matrix::data() const { return m_data; }
...
Matrix multiply_tile(const Matrix& a, const Matrix& b, size_t tile_size) {
    ...
    const std::vector<double>& A = a.data();
    const std::vector<double>& B_t = b_t.data();
    std::vector<double>& C = c.data();
...
PYBIND11_MODULE(_matrix, m) {
    py::class_<Matrix>(m, "Matrix")
        ...
        .def("__getitem__", [](const Matrix &m, std::pair<size_t, size_t> i) { return m(i.first, i.second); })
        .def("__setitem__", [](Matrix &m, std::pair<size_t, size_t> i, double v) { m(i.first, i.second) = v; });
