...
This project targets the **computational geometry** field, which can be used in:
...
- **Input**: 2D point sets in three formats:
  ...
  - Numpy array.
  - C++ `std::vector` of `Point` structs (e.g., `std::vector<Voronoi::Point>{{0.1f, 0.2f}}`).
- **Output**:
...
Modularization
The system is divided into four decoupled layers:
...
The project uses CMake 3.15+ for cross-platform building:
- **Build Commands**:
  ```bash
  mkdir build && cd build
  cmake .. -DCMAKE_BUILD_TYPE=Release -DENABLE_PARALLEL=ON -DBUILD_TESTS=ON
  make -j$(nproc)  # Compile with all CPU cores
  ```
- **Configuration Options**:
  - `-DENABLE_PARALLEL`: Enable multi-threaded optimizations (default: OFF).
  - `-DBUILD_TESTS`: Build unit tests (default: OFF).
  - `-DBUILD_PYTHON`: Build Python wrapper (default: ON).

 2. Lisence
- We use Apache 2.0 lisencing for this project!
