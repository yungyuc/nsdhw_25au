...
struct MemoryTracker {
    static std::atomic<std::size_t>& bytes_used() {
        static std::atomic<std::size_t> v{0};
        return v;
    }
    static std::atomic<std::size_t>& total_alloc() {
        static std::atomic<std::size_t> v{0};
        return v;
    }
    static std::atomic<std::size_t>& total_dealloc() {
        static std::atomic<std::size_t> v{0};
        return v;
    }
    static void on_alloc(std::size_t bytes) {
        if (!bytes) return;
        bytes_used().fetch_add(bytes, std::memory_order_relaxed);
        total_alloc().fetch_add(bytes, std::memory_order_relaxed);
    }
    static void on_dealloc(std::size_t bytes) {
        if (!bytes) return;
        total_dealloc().fetch_add(bytes, std::memory_order_relaxed);
        bytes_used().fetch_sub(bytes, std::memory_order_relaxed);
    }
};
...
public:
    ...
    void deallocate(T* p, std::size_t n) noexcept {
        const std::size_t bytes = n * sizeof(T);
        MemoryTracker::on_dealloc(bytes);
        ::operator delete(p);
    }
...
constexpr bool operator==(const CustomAllocator<T>&, const CustomAllocator<U>&) noexcept { return true; }
template <class T, class U>
constexpr bool operator!=(const CustomAllocator<T>&, const CustomAllocator<U>&) noexcept { return false; }
...
struct Matrix {
    std::size_t nrow;
    std::size_t ncol;
    std::vector<double, CustomAllocator<double>> buf; // row-major

    Matrix(std::size_t r, std::size_t c)
        : nrow(r), ncol(c), buf(r * c, 0.0) {}  // uses default-constructed CustomAllocator
...
PYBIND11_MODULE(_matrix, m) {
    m.doc() = "Matrix + GEMM implementations (naive/tiled/MKL) with tracked allocator";

    // Export memory tracking APIs required by HW4
    m.def("bytes", [](){
        return MemoryTracker::bytes_used().load(std::memory_order_relaxed);
    }, "Current live bytes allocated by Matrix buffers");
    m.def("allocated", [](){
        return MemoryTracker::total_alloc().load(std::memory_order_relaxed);
    }, "Total bytes ever allocated by Matrix buffers");
    m.def("deallocated", [](){
        return MemoryTracker::total_dealloc().load(std::memory_order_relaxed);
    }, "Total bytes ever deallocated by Matrix buffers");
