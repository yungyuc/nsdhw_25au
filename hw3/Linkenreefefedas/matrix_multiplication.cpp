...
Matrix multiply_mkl(const Matrix& A, const Matrix& B) {
    ...
    // MKL uses column-major order, we need to compute B^T * A^T = (A * B)^T
    // Then transpose the result, or equivalently:
    // cblas_dgemm computes C = alpha*op(A)*op(B) + beta*C
    // We want C = A * B where A is m x n, B is n x p, C is m x p
    // In row-major: C(i,j) = sum_k A(i,k) * B(k,j)
    // In col-major: this is equivalent to C^T = B^T * A^T
