...
namespace python
{
    ...
    public:
        ...
        using root_base_type = WrapBase<wrapper_type, wrapped_type, holder_type, wrapped_base_type>;
            ...
            : root_base_type(mod, pyname, pydoc, pybind11::buffer_protocol())
        {
            ...
            (*this)
                ...
                .def_property_readonly("array", [](Matrix &self)
                                       {
                // Determine strides for row-major layout
                size_t stride_row = self.ncol() * sizeof(double);
                size_t stride_col = sizeof(double);

                return py::array_t<double>(
                    { self.nrow(), self.ncol() },      // Shape
                    { stride_row, stride_col },        // Strides
                    self.data().data(),                // Pointer to data (std::vector::data())
                    py::cast(&self)                    // Parent object (keeps Matrix alive)
                ); })
